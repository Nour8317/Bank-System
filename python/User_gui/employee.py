...
class Employee(User):
    ...
    def view_loans_table(self, loans):
        ...
        def update_loan_state(event, loan_id, state_var):
            new_state = state_var.get()
            # Update the loan state in the database using the loan_id and new_state
            self.sql.update_loan_state(loan_id, new_state)
        ...
        for i, loan in enumerate(loans, start=1):
            ...
            state_option_menu = OptionMenu(loan_types_window, state_var, *state_options.values(), command=lambda event, loan_id=loan.loan_id, state_var=state_var: update_loan_state(event, loan_id, state_var))
